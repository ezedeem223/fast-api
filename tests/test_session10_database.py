...
def test_paginate_and_cursor_limits(session):
    ...
    cursor_data = query_helpers.cursor_paginate(base_query, cursor=None, limit=2, cursor_column="id", order_desc=False)
    assert cursor_data["count"] == 2
    assert cursor_data["has_next"] is True
    # use next_cursor
    cursor_data2 = query_helpers.cursor_paginate(base_query, cursor=cursor_data["next_cursor"], limit=3, cursor_column="id", order_desc=False)
    assert cursor_data2["count"] <= 3

    count = query_helpers.optimize_count_query(base_query.order_by(models.Post.id.desc()))
    assert count == 5
