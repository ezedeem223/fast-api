"""Test module for test services session6."""
import pytest
...
async def test_list_comments_filters_flagged_for_regular_users(session):
    """Test case for test list comments filters flagged for regular users."""
    user = _user(session)
...
async def test_list_comments_includes_flagged_for_moderators(session):
    """Test case for test list comments includes flagged for moderators."""
    user = _user(session)
...
def test_report_content_flags_comment(session):
    """Test case for test report content flags comment."""
...
def test_comment_soft_delete_marks_record(session):
    """Test case for test comment soft delete marks record."""
    owner = _user(session)
...
def test_set_best_answer_requires_owner(session):
    """Test case for test set best answer requires owner."""
    owner = _user(session)
    ...
    assert exc.value.detail == "Not authorized to set best answer"


def test_create_user_rejects_weak_password(monkeypatch, session):
    """Test case for test create user rejects weak password."""
    svc = users_service.UserService(session)
    monkeypatch.setattr(settings, "password_strength_required", True)
    monkeypatch.setattr(settings, "password_strength_min_score", 5)
    with pytest.raises(HTTPException) as exc:
        svc.create_user(
            schemas.UserCreate(email="weak@example.com", password="weak")
        )
    assert exc.value.status_code == 400
    assert exc.value.detail["message"] == "Password is too weak"


def test_privacy_custom_requires_payload(session):
    """Test case for test privacy custom requires payload."""
    svc = users_service.UserService(session)
    user = svc.create_user(
        schemas.UserCreate(
            email="p@example.com", password="StrongPass123!", username="user3"
        )
    )
...
def test_followers_settings_persist(session):
    """Test case for test followers settings persist."""
    svc = users_service.UserService(session)
    user = svc.create_user(
        schemas.UserCreate(
            email="f@example.com", password="StrongPass123!", username="user4"
        )
    )
...
def test_invite_members_skips_duplicates(session):
    """Test case for test invite members skips duplicates."""
    owner = _user(session)
...
def test_vote_in_poll_updates_existing_vote(session):
    """Test case for test vote in poll updates existing vote."""
    voter = _user(session)
