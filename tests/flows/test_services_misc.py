"""Test module for test session20 misc services."""
import pytest
...
def test_moderation_crud_and_duplicate(session, monkeypatch):
    """Test case for test moderation crud and duplicate."""
...
def test_business_service_and_reporting(session):
    """Test case for test business service and reporting."""
    ...
    reviewed = biz_service.review_business_verification(
        user_id=owner.id, decision=decision
    )
    assert reviewed.is_verified_business is True
    assert (
        reviewed.verification_status.value
        == schemas.VerificationStatus.APPROVED.value
    )

    approved = biz_service.list_business_verifications(
        status_filter=schemas.VerificationStatus.APPROVED
    )
    assert any(user.id == owner.id for user in approved)
...
def test_follow_service_unfollow_and_missing_user(session):
    """Test case for test follow service unfollow and missing user."""
    # Arrange: create two users and follow relationship.
    ...
    # Act: unfollow and verify record removal.
    svc.unfollow_user(
        current_user=u1,
        target_user_id=u2.id,
        background_tasks=None,
    ...
    # Assert: following a missing user raises.
    with pytest.raises(HTTPException):
        svc.follow_user(
            background_tasks=None,
            current_user=u1,
...
def test_wellness_service_errors(session):
    """Test case for test wellness service errors."""
