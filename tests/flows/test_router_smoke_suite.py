"""Test module for test session7 routers."""
...
def test_hashtag_happy_and_errors(session):
    """Test case for test hashtag happy and errors."""
    ...
    with make_client(session, current_user=user) as client:
        ...
        assert resp_follow.json()["message"] == "Hashtag followed successfully"
        resp_unfollow = client.post(f"/hashtags/unfollow/{hid}")
        assert resp_unfollow.status_code == 200
        assert resp_unfollow.json()["message"] == "Hashtag unfollowed successfully"

        # follow non-existent -> 404
        resp_missing = client.post("/hashtags/follow/9999")
        assert resp_missing.status_code == 404
        assert resp_missing.json()["detail"] == "Hashtag not found"
...
def test_category_admin_required_and_happy(session, monkeypatch):
    """Test case for test category admin required and happy."""
    admin = models.User(
    ...
    with make_client(session, current_user=user) as client_user:
        ...
        assert resp_forbidden.json()["detail"] == "Only admins can create categories"
    ...
    with make_client(session, current_user=admin) as client_admin:
        ...
        assert create_resp.json()["name"] == "tech"
        ...
        assert update_resp.json()["name"] == "science"
...
def test_statistics_vote_and_ban_overview(session, monkeypatch):
    """Test case for test statistics vote and ban overview."""
    user = models.User(
    ...
    monkeypatch.setattr(
        ...
        lambda db, uid: schemas.UserVoteAnalytics(
            total_posts=1,
...
def test_screen_share_start_end_and_unauthorized(session, monkeypatch):
    """Test case for test screen share start end and unauthorized."""
    # Arrange: create a call and stub the notification manager.
    ...
    # Act: start and end screen sharing as the caller.
    ...
    # Assert: unauthorized user cannot start screen share.
    with make_client(
...
def test_encrypted_session_create_and_update(session, monkeypatch):
    """Test case for test encrypted session create and update."""
    ...
    with make_client(session, current_user=user) as client:
        ...
        assert missing_resp.json()["detail"] == "User not found"
...
def test_social_auth_facebook_happy_and_invalid(monkeypatch, session):
    """Test case for test social auth facebook happy and invalid."""
    # Arrange: stub OAuth client and seed an existing user.
    class DummyResp:
    ...
    # Act/Assert: login and callback succeed, then fail on token error.
...
async def test_create_sticker_success_and_invalid_format(
    ...
    """Test case for test create sticker success and invalid format."""
    # Arrange: seed sticker pack/category and create a PNG upload.
    user = models.User(email="stick@example.com", hashed_password="x", is_verified=True)
    session.add(user)
    session.commit()
    session.refresh(user)

    pack = sticker_models.StickerPack(name="pack1", creator_id=user.id)
    ...
    # Assert: invalid formats are rejected.
...
def test_call_router_happy_and_missing_token(monkeypatch, session):
    """Test case for test call router happy and missing token."""
    # Arrange: create users and stub call service responses.
    ...
    with make_client(
        ...
        # Act: perform call lifecycle endpoints.
    ...
    # Assert: missing auth token rejects the request.
    with make_client(
        session,
        extra_overrides={
...
async def test_support_tickets_and_response(session):
    """Test case for test support tickets and response."""
