...
def test_profile_and_privacy_updates(session):
    ...
    updated = service.update_privacy_settings(
        user,
        schemas.UserPrivacyUpdate(privacy_level=schemas.PrivacyLevel.CUSTOM, custom_privacy={"allowed_users": [user.id]}),
    )
...
def test_followers_visibility_private(session, monkeypatch):
    owner = models.User(email="o23@example.com", hashed_password="x", is_verified=True, followers_visibility="private")
    requester = models.User(email="r23@example.com", hashed_password="x", is_verified=True)
    ...
    service = UserService(session)
    monkeypatch.setattr(service, "_resolve_followers_sort_column", lambda sort_by: models.Follow.created_at)
...
def test_followers_visibility_custom_allows_listed(session, monkeypatch):
    owner = models.User(email="o23c@example.com", hashed_password="x", is_verified=True, followers_visibility="custom", followers_custom_visibility={"allowed_users": []})
    allowed = models.User(email="allowed23@example.com", hashed_password="x", is_verified=True)
    follower = models.User(email="f23@example.com", hashed_password="x", is_verified=True)
...
def test_followers_visibility_custom_blocks_unlisted(session, monkeypatch):
    owner = models.User(email="o23d@example.com", hashed_password="x", is_verified=True, followers_visibility="custom", followers_custom_visibility={"allowed_users": []})
    requester = models.User(email="blocked23@example.com", hashed_password="x", is_verified=True)
    follower = models.User(email="f23b@example.com", hashed_password="x", is_verified=True)
...
def test_language_update_and_public_key(session, monkeypatch):
    user = models.User(email="lang23@example.com", hashed_password="x", is_verified=True, preferred_language="en")
    session.add(user)
    session.commit()
    session.refresh(user)
    ...
    resp = service.update_language_preferences(user, schemas.UserLanguageUpdate(preferred_language="en", auto_translate=True))
    assert resp["message"]

    # str key gets encoded
    updated = service.update_public_key(user, schemas.UserPublicKeyUpdate(public_key="abcd"))
    assert isinstance(updated.public_key, (bytes, bytearray))

    with pytest.raises(HTTPException):
        service.update_language_preferences(user, schemas.UserLanguageUpdate(preferred_language="xx", auto_translate=False))
...
def test_upload_profile_image_validates(monkeypatch, session, tmp_path):
    ...
    f.write_bytes(data)
    upload = UploadFile(filename="p.png", file=f.open("rb"), headers=Headers({"content-type": "image/png"}))
    path = service.upload_profile_image(user, upload)
    assert path.endswith("p.png")

    f2 = tmp_path.joinpath("y")
    f2.write_bytes(data)
    bad = UploadFile(filename="p.txt", file=f2.open("rb"), headers=Headers({"content-type": "text/plain"}))
    with pytest.raises(HTTPException):
        service.upload_profile_image(user, bad)


def test_verify_user_document(monkeypatch, session, tmp_path):
    user = models.User(email="doc23@example.com", hashed_password="x", is_verified=False)
    session.add(user)
    session.commit()
    session.refresh(user)
    service = UserService(session)
    ...
    f.write_bytes(b"pdf")
    upload = UploadFile(filename="doc.pdf", file=f.open("rb"), headers=Headers({"content-type": "application/pdf"}))
