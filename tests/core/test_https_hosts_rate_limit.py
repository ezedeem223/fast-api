"""Test module for test session2 https hosts rate."""
import importlib
...
def test_trusted_host_allows_and_denies():
    """Test case for test trusted host allows and denies."""
    app = FastAPI()
...
def test_host_redirect_middleware_http_to_https():
    """Test case for test host redirect middleware http to https."""
    app = FastAPI()
...
def test_host_redirect_middleware_https_pass_through():
    """Test case for test host redirect middleware https pass through."""
    app = FastAPI()
    app.add_middleware(HostRedirectMiddleware, allowed_hosts=["good.com"])
...
def test_host_redirect_middleware_invalid_host_first():
    """Test case for test host redirect middleware invalid host first."""
    app = FastAPI()
...
def test_rate_limiter_real_limits_and_429(reload_rate_limit):
    """Test case for test rate limiter real limits and 429."""
...
def test_rate_limiter_disabled_in_test_env(reload_rate_limit):
    """Test case for test rate limiter disabled in test env."""
...
def test_rate_limiter_keys_per_ip(reload_rate_limit):
    """Test case for test rate limiter keys per ip."""
    rl = reload_rate_limit("production")
    app = FastAPI()
    app.state.limiter = rl.limiter
    app.add_exception_handler(
