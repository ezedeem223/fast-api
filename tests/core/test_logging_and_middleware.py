"""Test module for test session12 logging and middleware."""
import importlib
import json
import logging
...
def test_setup_logging_creates_dir_and_sets_level(tmp_path):
    """Test case for test setup logging creates dir and sets level."""
...
def test_json_formatter_includes_fields():
    """Test case for test json formatter includes fields."""
...
async def test_ip_ban_middleware_allows_and_blocks(monkeypatch):
    """Test case for test ip ban middleware allows and blocks."""
    ...
    assert json.loads(allowed.body.decode())["ok"] is True
    blocked = await ip_ban.ip_ban_middleware(DummyRequest("1.1.1.1"), call_next)
    assert blocked.status_code == 403
    assert json.loads(blocked.body.decode())["detail"] == "Your IP address is banned"


@pytest.mark.asyncio
async def test_language_middleware_translates(monkeypatch):
    """Test case for test language middleware translates."""
...
def test_rate_limit_noop_and_handler(monkeypatch):
    """Test case for test rate limit noop and handler."""
