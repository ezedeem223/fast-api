"""Test module for test session1 telemetry logging."""
...
def test_setup_sentry_without_dsn(monkeypatch):
    ...
    monkeypatch.setattr(telemetry, "sentry_sdk", FakeSentry())
    result = telemetry.setup_sentry(dsn=None, environment="prod")
    assert result is None
...
def test_setup_sentry_missing_sdk(monkeypatch):
    ...
    result = telemetry.setup_sentry("dsn", environment="prod")
    assert result is None
