"""Test module for test session29 scheduling tasks."""
import asyncio
...
def test_repeat_every_guard_skips_in_test_env(monkeypatch):
    """Test case for test repeat every guard skips in test env."""
    called = {"hit": False}
...
def test_register_startup_tasks_idempotent(monkeypatch):
    """Test case for test register startup tasks idempotent."""
    app = FastAPI()
...
def test_notification_cleanup_and_retry_tasks(session, monkeypatch):
    """Test case for test notification cleanup and retry tasks."""
    monkeypatch.setattr(
        tasks,
        "SessionLocal",
...
def test_cleanup_expired_reels_task(session, monkeypatch):
    """Test case for test cleanup expired reels task."""
