"""Test module for test logging middleware session11."""
...
def build_app_with_logging(extra_middleware=None):
    """Helper for build app with logging."""
    app = FastAPI()
...
def test_logging_middleware_sets_request_id_and_logs(monkeypatch):
    """Test case for test logging middleware sets request id and logs."""
    logged = []
    tokens_reset = []
...
def test_logging_middleware_sets_span_attributes_and_user(monkeypatch):
    """Test case for test logging middleware sets span attributes and user."""
    # Arrange: stub tracer and logging hooks.
    span_attrs = {}
    ...
    # Assert: tracing attributes and logged metadata are populated.
    assert resp.status_code == 200
    req_id = resp.headers.get("X-Request-ID")
