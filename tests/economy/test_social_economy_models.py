"""Test module for test social economy."""
...
def test_social_economy_initial_scoring(
    ...
    """Verify social economy scoring updates after creating a post."""
    user_id = test_user["id"]
    user_obj = db_session.query(User).filter(User.id == user_id).first()

    # Arrange: capture baseline social credits before post creation.
    ...
    # Assert: derived scores are positive and credits do not decrease.
