"""Test module for test auth session25."""
import datetime
...
def test_login_success(monkeypatch, session):
    """Test case for test login success."""
    user = _user(session)
...
def test_login_locked_account(session, monkeypatch):
    """Test case for test login locked account."""
    ...
    assert exc.value.detail == "Account is locked. Please try again later."


def test_login_invalid_credentials_increments(session, monkeypatch):
    """Test case for test login invalid credentials increments."""
...
async def test_refresh_token_invalid_format(session):
    """Test case for test refresh token invalid format."""
    with pytest.raises(auth.HTTPException) as exc:
...
async def test_refresh_token_valid(monkeypatch, session):
    """Test case for test refresh token valid."""
