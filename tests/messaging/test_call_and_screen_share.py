"""Test module for test session11 call and screen share."""
...
def caller_client(authorized_client, test_user):
    """Pytest fixture for caller_client."""
...
def test_start_call_sets_pending_and_notifies(
    ...
    """Test case for test start call sets pending and notifies."""
    sent = {}
...
def test_update_call_ended_marks_end_and_screen_share(
    ...
    """Test case for test update call ended marks end and screen share."""
    resp = caller_client.post(
        "/calls/",
...
def test_update_call_forbidden_for_non_participant(
    ...
    """Test case for test update call forbidden for non participant."""
    ...
    assert res.json()["detail"] == "Not authorized to update this call"


def test_start_screen_share_sets_active_and_notifies(
    monkeypatch, caller_client, session, test_user2
):
    """Test case for test start screen share sets active and notifies."""
    messages = {}
...
def test_start_screen_share_rejects_duplicate_active(
    ...
    """Test case for test start screen share rejects duplicate active."""
    call_resp = caller_client.post(
        "/calls/", json={"receiver_id": test_user2["id"], "call_type": "video"}
    )
    call_id = call_resp.json()["id"]
    ...
    assert res.json()["detail"] == "There's already an active screen share in this call"


def test_update_screen_share_status_ended(
    monkeypatch, caller_client, session, test_user, test_user2
):
    """Test case for test update screen share status ended."""
    notified = {}
...
def test_end_screen_share_requires_owner(
    ...
    """Test case for test end screen share requires owner."""
    ...
    assert res.status_code == 403
    assert res.json()["detail"] == "Not authorized to end this screen share session"


@pytest.mark.asyncio
async def test_handle_call_data_forwards_valid_types(monkeypatch):
    """Test case for test handle call data forwards valid types."""
    sent = {}
...
async def test_handle_call_disconnect_sets_status_and_notifies(monkeypatch, session):
    """Test case for test handle call disconnect sets status and notifies."""
