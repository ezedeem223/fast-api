...
async def test_message_service_send_list_update_delete(session, monkeypatch):
    """Test case for test message service send list update delete."""
...
async def test_message_service_send_file_and_ordering(session, monkeypatch, tmp_path):
    """Test case for test message service send file and ordering."""
    service = MessageService(session)
...
async def test_message_edit_delete_window_and_permissions(session, monkeypatch):
    """Test case for test message edit delete window and permissions."""
    service = MessageService(session)
    ...
    assert exc.value.status_code == 400
    assert exc.value.detail == "Delete window has expired"
...
async def test_message_list_pagination_and_ordering(session, monkeypatch):
    """Test case for test message list pagination and ordering."""
    service = MessageService(session)
...
async def test_send_file_validation_and_virus_scan(session, monkeypatch, tmp_path):
    """Test case for test send file validation and virus scan."""
    service = MessageService(session)
...
async def test_mark_message_as_read_and_get_message_permissions(session, monkeypatch):
    """Test case for test mark message as read and get message permissions."""
    # Arrange: create a message and stub notification side effects.
    service = MessageService(session)
    ...
    # Act/Assert: outsiders cannot mark/read or fetch messages.
    with pytest.raises(HTTPException) as exc:
    ...
    # Assert: receiver can mark as read and access.
...
async def test_create_audio_message_validation(session, monkeypatch, tmp_path):
    """Test case for test create audio message validation."""
    service = MessageService(session)
...
def test_get_conversation_messages_membership_guard(session):
    """Test case for test get conversation messages membership guard."""
    service = MessageService(session)
    ...
    assert exc.value.status_code == 403
    assert exc.value.detail == "You are not part of this conversation"


def test_banned_word_service_crud(session, monkeypatch):
    """Test case for test banned word service crud."""
...
def test_reporting_negative_paths(session, monkeypatch):
    """Test case for test reporting negative paths."""
