"""Test module for test session10 messaging router."""
...
async def test_text_message_creates_stats(monkeypatch, session):
    """Test case for test text message creates stats."""
...
async def test_send_file_oversized(monkeypatch, session):
    """Test case for test send file oversized."""
    service = MessageService(session)
...
async def test_unknown_sticker_rejected(session):
    """Test case for test unknown sticker rejected."""
    service = MessageService(session)
    ...
    assert exc.value.detail == "Sticker not found or not approved"


def _stub_signal_protocol(monkeypatch):
    """Helper for  stub signal protocol."""
    class StubDH:
        def public_key(self):
            return self
...
def test_create_encrypted_session_stores_keys(
    ...
    """Test case for test create encrypted session stores keys."""
...
def test_update_session_persists_chain_key(monkeypatch, authorized_client, test_user2):
    """Test case for test update session persists chain key."""
    _stub_signal_protocol(monkeypatch)
...
def test_update_session_missing_field(monkeypatch, authorized_client, test_user2):
    """Test case for test update session missing field."""
    _stub_signal_protocol(monkeypatch)
    created = authorized_client.post(
        "/sessions", json={"other_user_id": test_user2["id"]}
    ).json()
    ...
    assert any("chain_key" in err["loc"] for err in detail)


def test_conversation_stats_response_and_files(session):
    """Test case for test conversation stats response and files."""
    # Arrange: create a conversation with two messages and attachments.
    sender = _user(session, "stats10@example.com")
    recv = _user(session, "stats210@example.com")
    service = MessageService(session)
    conv_id = service._get_or_create_direct_conversation(sender.id, recv.id)
    msg1 = models.Message(
        sender_id=sender.id,
        receiver_id=recv.id,
    ...
    # Act: update stats after each message.
    ...
    # Assert: aggregated stats reflect messages, files, and emojis.
    stats = (
        session.query(models.ConversationStatistics)
