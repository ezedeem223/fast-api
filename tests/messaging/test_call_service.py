"""Test module for test session5 call service."""
...
class _StubNotifications:
    """Test class for _StubNotifications."""
    def __init__(self):
        self.sent = []
...
async def test_start_call_success_and_limits(session):
    """Test case for test start call success and limits."""
...
async def test_update_call_status_permissions_and_end_share(session):
    """Test case for test update call status permissions and end share."""
    # Arrange: create a call with an active screen-share session.
    service = CallService(session)
    ...
    # Act/Assert: permission checks for outsiders and missing calls.
    with pytest.raises(HTTPException) as exc:
    ...
    # Act: end the call and verify share is closed.
...
def test_active_calls_and_quality_update(session):
    """Test case for test active calls and quality update."""
    service = CallService(session)
