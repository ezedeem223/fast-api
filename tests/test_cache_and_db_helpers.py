...
async def test_cached_query_respects_disabled_cache(monkeypatch):
    redis_cache.cache_manager = redis_cache.RedisCache()
    async def query_fn(): return "fresh"
...
def test_get_db_yields_and_closes(monkeypatch):
    ...
    db_session.SessionLocal = db_session.sessionmaker(autocommit=False, autoflush=False, bind=eng)
...
def test_cursor_paginate_edges():
    ...
    class FakeQuery:
        ...
        def limit(self, n): return SimpleNamespace(all=lambda: self.items[:n])
        @property
        def column_descriptions(self): return [{"entity": Item}]
