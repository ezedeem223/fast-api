...
def _load_analytics_with_stubbed_transformers():
    ...
    dummy.pipeline = lambda *a, **k: (lambda text: [{"label": "POSITIVE", "score": 1.0}])
    dummy.AutoTokenizer = types.SimpleNamespace(from_pretrained=lambda *a, **k: object())
    dummy.AutoModelForSequenceClassification = types.SimpleNamespace(from_pretrained=lambda *a, **k: object())
...
def test_log_analysis_event_failure_with_details(caplog):
    ...
    assert any(rec.message == "analytics.failure" and getattr(rec, "error", "") == "boom" for rec in caplog.records)
...
def test_merge_stats_combines_numbers_and_handles_none():
    merged = analytics.merge_stats({"views": 1, "likes": 2}, {"views": 2, "comments": 1})
