"""Test module for test session5 block moderation."""
...
def test_appeal_creation_and_duplicate(
    ...
    assert (
        dup.json()["detail"] == "An appeal for this block is already pending"
    )

    # non-owner cannot appeal
    forbidden = client.post("/block/appeal", json=appeal_payload, headers=headers_user2)
    assert forbidden.status_code == status.HTTP_403_FORBIDDEN
    assert forbidden.json()["detail"] == "You can only appeal your own blocks"
