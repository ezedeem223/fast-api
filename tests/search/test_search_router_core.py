"""Test module for test search router session27."""
import pytest
...
async def test_search_uses_typesense_ordering(monkeypatch, session):
    """Test case for test search uses typesense ordering."""
    user = _user(session)
...
async def test_search_typesense_failure_falls_back(monkeypatch, session):
    """Test case for test search typesense failure falls back."""
    user = _user(session)
...
async def test_autocomplete_caches_results(monkeypatch, session):
    """Test case for test autocomplete caches results."""
    _user(session)
...
async def test_record_search_increments_frequency(monkeypatch, session):
    """Test case for test record search increments frequency."""
    user = _user(session)
    term = "searchme"
