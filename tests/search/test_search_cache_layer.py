"""Test module for test session14 search cache layer."""
...
def test_popular_recent_cache_roundtrip(monkeypatch):
    """Test case for test popular recent cache roundtrip."""
    dummy = DummyRedis()
    monkeypatch.setattr(
        search_cache,
        "settings",
...
def test_user_cache_invalidation(monkeypatch):
    """Test case for test user cache invalidation."""
    dummy = DummyRedis()
    dummy.store = {
...
def test_fail_open_without_redis(monkeypatch):
    """Test case for test fail open without redis."""
    monkeypatch.setattr(
