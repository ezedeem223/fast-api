"""Test module for test session16 utils network."""
...
def test_get_client_ip_prefers_forwarded():
    """Test case for test get client ip prefers forwarded."""
    req = SimpleNamespace(
...
def test_is_ip_banned_handles_expiry(session):
    """Test case for test is ip banned handles expiry."""
    active = models.IPBan(
        ip_address="10.0.0.1",
        reason="x",
...
def test_detect_ip_evasion_private_vs_public(session):
    """Test case for test detect ip evasion private vs public."""
...
def test_parse_json_response_and_safe_request(monkeypatch):
    """Test case for test parse json response and safe request."""
    class Resp:
        def json(self):
            return {"ok": True}
