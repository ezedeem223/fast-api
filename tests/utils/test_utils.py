"""Test module for test utils session5."""
import json
...
def test_update_post_score_with_missing_values(session):
    """Test case for test update post score with missing values."""
    post = utils_analytics.models.Post(
        owner_id=1, title="t", content="c", is_safe_content=True
    )
    session.add(post)
    session.commit()
...
def test_create_default_categories_no_duplicates(session):
    """Test case for test create default categories no duplicates."""
...
def test_offensive_classifier_stub_in_test(monkeypatch):
    """Test case for test offensive classifier stub in test."""
...
def test_get_client_ip_header(monkeypatch):
    """Test case for test get client ip header."""
    request = type(
        "Req",
        (),
        {
            "headers": {"X-Forwarded-For": "1.1.1.1,2.2.2.2"},
            "client": type("c", (), {"host": "3.3.3.3"}),
        },
    )()
...
def test_detect_ip_evasion_handles_programming_error(monkeypatch):
    """Test case for test detect ip evasion handles programming error."""
    class BoomSession:
        def query(self, *_, **__):
...
def test_links_update_link_preview_no_preview(monkeypatch):
    """Test case for test links update link preview no preview."""
...
def test_link_preview_unsupported_content_type(monkeypatch):
    """Test case for test link preview unsupported content type."""
    class DummyResponse:
        status_code = 200
    ...
    monkeypatch.setattr(
        link_preview.requests, "get", lambda u, timeout=5: DummyResponse()
    )
...
def test_search_cache_ttl(monkeypatch):
    """Test case for test search cache ttl."""
    class FakeRedis:
        def __init__(self):
            self.values = {}
