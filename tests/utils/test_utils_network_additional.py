"""Test module for test utils network session16."""
...
def test_is_ip_banned_active_and_expired(monkeypatch):
    """Test case for test is ip banned active and expired."""
...
def test_parse_json_response(monkeypatch):
    """Test case for test parse json response."""
    class DummyResp:
        def json(self):
            return {"ok": True}
...
def test_with_retry_success_and_fail(monkeypatch):
    """Test case for test with retry success and fail."""
    attempts = {"count": 0}
...
def test_safe_request(monkeypatch):
    """Test case for test safe request."""
    def bad():
