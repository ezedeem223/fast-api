"""Test module for test admin dashboard session28."""
import pytest
...
async def test_admin_stats_overview_routes():
    """Test case for test admin stats overview routes."""
...
def test_get_users_sort_and_role_update(monkeypatch):
    """Test case for test get users sort and role update."""
    with TestingSessionLocal() as db:
        admin = seed_admin(db)
        user1 = models.User(
...
def test_ban_statistics_and_common_reasons():
    """Test case for test ban statistics and common reasons."""
    with TestingSessionLocal() as db:
        admin = seed_admin(db)
...
def test_admin_users_requires_admin_and_auth(monkeypatch):
    """Test case for test admin users requires admin and auth."""
    with TestingSessionLocal() as db:
        ...
        assert resp2.json()["detail"] == "Unauthorized"
