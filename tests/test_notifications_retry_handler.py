...
def _notification(session, user_id, status=notification_models.NotificationStatus.FAILED, retry_count=0):
...
@pytest.mark.asyncio
async def test_schedule_retry_adds_task_when_background_tasks_present(session, test_user):
    bg = MagicMock()
...
@pytest.mark.asyncio
async def test_schedule_retry_runs_immediately_when_no_background_tasks(session, test_user):
...
async def test_retry_notification_waits_and_delivers(monkeypatch, session, test_user):
    handler = NotificationRetryHandler(session, background_tasks=None)
    n = _notification(session, test_user["id"], status=notification_models.NotificationStatus.RETRYING, retry_count=0)

    async def fake_deliver(notification_id, delay):
        await asyncio.sleep(0)
        notif = get_model_by_id(session, notification_models.Notification, notification_id)
        if notif:
