"""Test module for test notifications email batch session18."""
...
async def test_send_email_notification_skips_in_test_env(monkeypatch):
    """Test case for test send email notification skips in test env."""
    ...
    result = await email.send_email_notification(
...
async def test_send_email_notification_requires_recipient(monkeypatch):
    """Test case for test send email notification requires recipient."""
...
async def test_send_email_notification_skips_without_credentials(monkeypatch):
    """Test case for test send email notification skips without credentials."""
    monkeypatch.setattr(settings, "environment", "production")
    monkeypatch.setenv("DISABLE_EXTERNAL_NOTIFICATIONS", "0")
...
async def test_send_email_notification_sends_when_configured(monkeypatch):
    """Test case for test send email notification sends when configured."""
    monkeypatch.setattr(settings, "environment", "production")
    monkeypatch.setenv("DISABLE_EXTERNAL_NOTIFICATIONS", "0")
...
async def test_schedule_email_notification_by_id_fetches_and_sends(monkeypatch):
    """Test case for test schedule email notification by id fetches and sends."""
    # Arrange: stub DB lookups and email delivery.
    ...
    # Act: schedule the notification and run the task immediately.
    email.schedule_email_notification_by_id(42, delay=0)
    assert tasks, "Expected the coroutine to be scheduled"
    await tasks[0]

    # Assert: message delivered and session closed.
...
async def test_send_mention_notification_builds_message(monkeypatch):
    """Test case for test send mention notification builds message."""
    captured = {}
...
async def test_send_login_notification_builds_message(monkeypatch):
    """Test case for test send login notification builds message."""
    captured = {}
...
async def test_batcher_add_flushes_on_size(monkeypatch):
    """Test case for test batcher add flushes on size."""
...
async def test_batcher_add_flushes_on_time(monkeypatch):
    """Test case for test batcher add flushes on time."""
...
async def test_batcher_digest_flushes_on_size(monkeypatch):
    """Test case for test batcher digest flushes on size."""
...
async def test_flush_digests_sends_all(monkeypatch):
    """Test case for test flush digests sends all."""
...
async def test_process_batch_routes_channels(monkeypatch):
    """Test case for test process batch routes channels."""
    batcher = NotificationBatcher()
...
async def test_send_batch_emails_groups_by_recipient(monkeypatch):
    """Test case for test send batch emails groups by recipient."""
    batcher = NotificationBatcher()
    sent = []
...
async def test_send_digest_email_formats_content(monkeypatch):
    """Test case for test send digest email formats content."""
    batcher = NotificationBatcher()
    sent = []
