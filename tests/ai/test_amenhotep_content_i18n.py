"""Test module for test session6 amenhotep content i18n."""
...
@pytest.mark.asyncio
async def test_amenhotep_fact_check_response_uses_verified_fact(
    monkeypatch, session
):
    """Test case for test amenhotep fact check response uses verified fact."""
    _stub_hf(monkeypatch)
    bot = AmenhotepAI()
    bot.knowledge_base = {}
    monkeypatch.setattr(bot, "_format_royal_response", lambda resp, **_: resp)
    monkeypatch.setattr(
        bot,
    ...
    submitter = session.query(models.User).first()
    if not submitter:
    ...
    response = await bot.get_response(
        ...
        message="The earth orbits the sun",
        db=session,
    )
    assert "The earth orbits the sun" in response
...
def test_content_filter_warn_and_ban(session):
    """Test case for test content filter warn and ban."""
...
def test_content_filter_empty(session):
    """Test case for test content filter empty."""
...
def test_content_filter_word_boundary_sensitive(session):
    """Test case for test content filter word boundary sensitive."""
...
def test_get_locale_supported(monkeypatch):
    """Test case for test get locale supported."""
...
def test_get_locale_unsupported(monkeypatch):
    """Test case for test get locale unsupported."""
...
def test_translate_and_detect_fallback(monkeypatch):
    """Test case for test translate and detect fallback."""
...
async def test_list_posts_translate_flag(session, monkeypatch):
    """Test case for test list posts translate flag."""
    from app.services.posts.post_service import PostService

    # Arrange: user prefers Spanish and a post is stored in English.
    ...
    # Act: list with translation enabled.
    ...
    # Assert: disabling translation returns raw content.
