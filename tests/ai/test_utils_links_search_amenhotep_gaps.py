"""Test module for test utils links search amenhotep gaps."""
...
def test_extract_links_variations():
    """Test case for test extract links variations."""
...
def test_spell_suggestions_ignore_invalid_tokens(monkeypatch):
    """Test case for test spell suggestions ignore invalid tokens."""
    class DummySpell:
...
def _build_app(session, user, monkeypatch, default_lang="en"):
    ...
    async def fake_get_shared(_app):
        return DummyAmenhotep()
...
def test_ask_invalid_input_returns_422(session, monkeypatch):
    """Test case for test ask invalid input returns 422."""
    ...
    with TestClient(app) as client:
        ...
        assert any("message" in err["loc"] for err in detail)


def test_ask_rejects_empty_text(session, monkeypatch):
    """Test case for test ask rejects empty text."""
    user = _create_user(session)
    app = _build_app(session, user, monkeypatch)
    ...
    with TestClient(app) as client:
        ...
        if isinstance(detail, list):
            ...
            assert detail == "Message cannot be empty"


def test_ask_language_fallback_uses_default(session, monkeypatch):
    """Test case for test ask language fallback uses default."""
