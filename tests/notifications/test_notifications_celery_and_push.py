"""Test module for test notifications celery and push."""
...
def test_send_push_notification_no_devices_noop(monkeypatch):
    """Test case for test send push notification no devices noop."""
    # Arrange: create fake session with no active devices.
    class FakeNotif:
        def __init__(self):
            self.id = 1
            self.user_id = 1
    ...
    # Act/Assert: task completes and commits without sending.
    send_push_notification_task(fake_session, 1)
    assert fake_session.committed is True


def test_send_push_notification_sends(monkeypatch):
    """Test case for test send push notification sends."""
    # Arrange: create fake session with an active device.
    class FakeNotif:
        def __init__(self):
            self.id = 1
            self.user_id = 1
    ...
    monkeypatch.setattr("app.modules.notifications.tasks.legacy_models", FakeLegacy)
    fake_session = FakeSession()
    # Act/Assert: task triggers send and commits.
    send_push_notification_task(fake_session, 1)
...
def test_send_push_notification_logs_on_error(monkeypatch, caplog):
    """Test case for test send push notification logs on error."""
    class FakeNotif:
        def __init__(self):
            self.id = 1
            self.user_id = 1
...
def test_celery_eager_mode_settings(monkeypatch):
    """Test case for test celery eager mode settings."""
...
def test_celery_cleanup_old_notifications_closes_db(monkeypatch):
    """Test case for test celery cleanup old notifications closes db."""
...
def test_celery_cleanup_old_notifications_handles_exception(monkeypatch):
    """Test case for test celery cleanup old notifications handles exception."""
    from app import celery_worker
...
def test_celery_cleanup_old_notifications_multiple_calls(monkeypatch):
    """Test case for test celery cleanup old notifications multiple calls."""
    from app import celery_worker
