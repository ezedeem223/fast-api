"""Test module for test notifications session2 tasks."""
...
def test_cleanup_old_notifications_wrapper_closes_session(monkeypatch):
    """Test case for test cleanup old notifications wrapper closes session."""
    closed = {"value": False}
...
def test_process_scheduled_notifications_marks_delivered_and_calls_delay(monkeypatch):
    """Test case for test process scheduled notifications marks delivered and calls delay."""
    # Arrange: stub a due notification and a fake celery delay handler.
    called = {"ids": []}
    ...
    # Act: process scheduled notifications.
    celery_worker.process_scheduled_notifications()

    # Assert: notification is marked delivered and queued.
    assert called["ids"] == [1]
...
def test_deliver_notification_task_success_calls_channels(session, test_user):
    """Test case for test deliver notification task success calls channels."""
...
def test_deliver_notification_task_missing_notification_is_noop(session):
    """Test case for test deliver notification task missing notification is noop."""
    ...
    deliver_notification_task(session, 999999, email_sender, push_sender)
    assert calls["email"] == 0
    assert calls["push"] == 0


def test_send_push_notification_task_logs_on_error(monkeypatch, caplog):
    """Test case for test send push notification task logs on error."""
    caplog.clear()
...
async def test_notification_batcher_groups_three_emails(monkeypatch):
    """Test case for test notification batcher groups three emails."""
    sent = {}
...
async def test_send_email_notification_raises_on_timeout(monkeypatch):
    """Test case for test send email notification raises on timeout."""
    monkeypatch.setattr(
...
async def test_realtime_prunes_broken_connection(monkeypatch):
    """Test case for test realtime prunes broken connection."""
