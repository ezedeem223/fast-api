"""Test module for test notifications tasks and ws."""
...
async def test_deliver_scheduled_notification_sends_and_closes(
    ...
    """Test case for test deliver scheduled notification sends and closes."""
...
async def test_deliver_scheduled_notification_missing_logs_and_closes(
    ...
    """Test case for test deliver scheduled notification missing logs and closes."""
    close_flag = {"closed": False}
...
async def test_send_bulk_notifications_counts_success_and_failures(
    ...
    """Test case for test send bulk notifications counts success and failures."""
    successes = [
...
async def test_send_real_time_notification_wraps_string(monkeypatch):
    """Test case for test send real time notification wraps string."""
    sent = {}
...
async def test_send_real_time_notification_passes_dict(monkeypatch):
    """Test case for test send real time notification passes dict."""
    sent = {}
...
async def test_send_real_time_notification_exception_logged(monkeypatch, caplog):
    """Test case for test send real time notification exception logged."""
...
async def test_websocket_empty_message_disconnect(monkeypatch):
    """Test case for test websocket empty message disconnect."""
...
async def test_websocket_disconnect_clean(monkeypatch):
    """Test case for test websocket disconnect clean."""
    connect_mock = AsyncMock(return_value=True)
    disconnect_mock = AsyncMock()
    send_mock = AsyncMock()
    monkeypatch.setattr(ws_manager, "connect", connect_mock)
...
async def test_websocket_general_exception_logs(monkeypatch):
    """Test case for test websocket general exception logs."""
    connect_mock = AsyncMock(return_value=True)
    disconnect_mock = AsyncMock()
    send_mock = AsyncMock()
    monkeypatch.setattr(ws_manager, "connect", connect_mock)
...
async def test_websocket_no_repeat_after_empty(monkeypatch):
    """Test case for test websocket no repeat after empty."""
    connect_mock = AsyncMock(return_value=True)
    disconnect_mock = AsyncMock()
    send_mock = AsyncMock(side_effect=AssertionError("should not send"))
    monkeypatch.setattr(ws_manager, "connect", connect_mock)
...
async def test_websocket_auth_requires_token(monkeypatch):
    """Test case for test websocket auth requires token."""
