"""Test module for test notifications session1 service."""
import asyncio
...
def test_notification_repository_filters_and_counts(session, test_user):
    """Test case for test notification repository filters and counts."""
...
def test_notification_repository_mark_all_and_cleanup(session, test_user):
    """Test case for test notification repository mark all and cleanup."""
    repo = NotificationRepository(session)
    n1 = repo.create_notification(
        user_id=test_user["id"],
        content="n1",
...
def test_notification_analytics_service(session, test_user):
    """Test case for test notification analytics service."""
    now = datetime.now(timezone.utc)
...
async def test_notification_service_respects_preferences(
    ...
    """Test case for test notification service respects preferences."""
...
def test_notification_service_mark_notifications_seen(session, test_user):
    """Test case for test notification service mark notifications seen."""
...
async def test_websocket_endpoint_happy_and_empty_message(monkeypatch):
    """Test case for test websocket endpoint happy and empty message."""
...
async def test_connection_manager_broadcast_and_cleanup(monkeypatch):
    """Test case for test connection manager broadcast and cleanup."""
...
async def test_send_real_time_notification_builds_payload(monkeypatch):
    """Test case for test send real time notification builds payload."""
    from app.modules.notifications import realtime
...
async def test_delivery_manager_retry_on_failure(monkeypatch, session, test_user):
    """Test case for test delivery manager retry on failure."""
...
async def test_delivery_manager_final_failure_records_reason(
    ...
    """Test case for test delivery manager final failure records reason."""
    manager = NotificationDeliveryManager(session, background_tasks=MagicMock())
...
async def test_notification_service_retry_failed_notification(
    ...
    """Test case for test notification service retry failed notification."""
    notif = _make_notification(
        test_user["id"],
...
async def test_notification_retry_handler_handles_limits(
    ...
    """Test case for test notification retry handler handles limits."""
    bg = MagicMock()
...
async def test_delivery_success_sets_delivered(monkeypatch, session, test_user):
    """Test case for test delivery success sets delivered."""
...
async def test_delivery_failure_records_reason_and_respects_next_retry(
    ...
    """Test case for test delivery failure records reason and respects next retry."""
    delivery_status_cache.clear()
    manager = NotificationDeliveryManager(session, background_tasks=MagicMock())
...
async def test_exceeding_retries_sets_failed_and_archivable(
    ...
    """Test case for test exceeding retries sets failed and archivable."""
    delivery_status_cache.clear()
    manager = NotificationDeliveryManager(session, background_tasks=MagicMock())
    manager.max_retries = 1
    prefs = notification_models.NotificationPreferences(
...
async def test_delivery_deduplicates_on_cache_hit(monkeypatch, session, test_user):
    """Test case for test delivery deduplicates on cache hit."""
    delivery_status_cache.clear()
    manager = NotificationDeliveryManager(session, background_tasks=MagicMock())
    prefs = notification_models.NotificationPreferences(
        user_id=test_user["id"],
...
async def test_delivery_cache_miss_then_hit(monkeypatch, session, test_user):
    """Test case for test delivery cache miss then hit."""
    delivery_status_cache.clear()
    manager = NotificationDeliveryManager(session, background_tasks=MagicMock())
    prefs = notification_models.NotificationPreferences(
        user_id=test_user["id"],
...
async def test_delivery_prefers_realtime_when_push_unavailable(
    ...
    """Test case for test delivery prefers realtime when push unavailable."""
...
async def test_delivery_uses_email_when_other_channels_disabled(
    ...
    """Test case for test delivery uses email when other channels disabled."""
    notification_cache.clear()
    delivery_status_cache.clear()
    manager = NotificationDeliveryManager(session, background_tasks=MagicMock())
    prefs = notification_models.NotificationPreferences(
...
async def test_archive_missing_notification_raises(monkeypatch, session, test_user):
    """Test case for test archive missing notification raises."""
    service = NotificationService(session)
...
async def test_update_preferences_then_skip_channel(monkeypatch, session, test_user):
    """Test case for test update preferences then skip channel."""
    service = NotificationService(session, background_tasks=MagicMock())
    prefs = notification_models.NotificationPreferences(
        user_id=test_user["id"],
        categories_preferences={},
...
async def test_notification_feed_pagination_and_filters(session, test_user):
    """Test case for test notification feed pagination and filters."""
    service = NotificationService(session)
