"""Test module for test notifications retry handler."""
import asyncio
...
async def test_schedule_retry_adds_task_when_background_tasks_present(
    ...
    """Test case for test schedule retry adds task when background tasks present."""
    bg = MagicMock()
...
async def test_schedule_retry_runs_immediately_when_no_background_tasks(
    ...
    """Test case for test schedule retry runs immediately when no background tasks."""
...
async def test_no_retry_scheduled_when_max_retries_reached(session, test_user):
    """Test case for test no retry scheduled when max retries reached."""
    handler = NotificationRetryHandler(session, background_tasks=None)
...
async def test_retry_notification_waits_and_delivers(monkeypatch, session, test_user):
    """Test case for test retry notification waits and delivers."""
    handler = NotificationRetryHandler(session, background_tasks=None)
    n = _notification(
        session,
        test_user["id"],
