"""Test module for test notifications service channels cache."""
...
def _clear_delivery_cache():
    """Pytest fixture for _clear_delivery_cache."""
...
async def test_delivery_cache_hit_returns_without_sending(
    ...
    """Test case for test delivery cache hit returns without sending."""
...
async def test_delivery_cache_cleared_triggers_send_and_sets_success(
    ...
    """Test case for test delivery cache cleared triggers send and sets success."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=True,
        push_notifications=False,
...
async def test_delivery_cache_updates_to_failure_on_send_error(
    ...
    """Test case for test delivery cache updates to failure on send error."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=True,
        push_notifications=False,
...
async def test_push_channel_success_sets_delivered(monkeypatch, session, test_user):
    """Test case for test push channel success sets delivered."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
...
async def test_push_channel_exception_logged_and_marks_failed(
    ...
    """Test case for test push channel exception logged and marks failed."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=False,
        push_notifications=True,
...
async def test_push_channel_no_devices_returns_without_error(
    ...
    """Test case for test push channel no devices returns without error."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=False,
        push_notifications=True,
...
async def test_push_channel_status_and_log_consistent(monkeypatch, session, test_user):
    """Test case for test push channel status and log consistent."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=False,
        push_notifications=True,
...
async def test_email_channel_missing_email_logs_warning(
    ...
    """Test case for test email channel missing email logs warning."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=True,
        push_notifications=False,
...
async def test_email_channel_timeout_logged(monkeypatch, session, test_user, caplog):
    """Test case for test email channel timeout logged."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=True,
        push_notifications=False,
...
async def test_email_channel_bad_credentials_logged(
    ...
    """Test case for test email channel bad credentials logged."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=True,
        push_notifications=False,
...
async def test_realtime_channel_success(monkeypatch, session, test_user):
    """Test case for test realtime channel success."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=False,
        push_notifications=False,
...
async def test_realtime_channel_exception_logs(monkeypatch, session, test_user, caplog):
    """Test case for test realtime channel exception logs."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=False,
        push_notifications=False,
...
async def test_realtime_channel_accepts_dict_payload(monkeypatch, session, test_user):
    """Test case for test realtime channel accepts dict payload."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=False,
        push_notifications=False,
...
async def test_email_channel_success_adds_log(monkeypatch, session, test_user):
    """Test case for test email channel success adds log."""
    mgr = _make_delivery_manager(session)
    prefs = SimpleNamespace(
        email_notifications=True,
        push_notifications=False,
