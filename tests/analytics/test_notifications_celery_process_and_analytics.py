"""Test module for test notifications celery process and analytics."""
...
def test_celery_process_scheduled_notifications_enqueues(
    ...
    """Test case for test celery process scheduled notifications enqueues."""
...
def test_celery_process_scheduled_notifications_no_pending(monkeypatch):
    """Test case for test celery process scheduled notifications no pending."""
    calls = {"commit": 0}
...
def test_celery_process_scheduled_notifications_closes_db(monkeypatch):
    """Test case for test celery process scheduled notifications closes db."""
    class FakeSession:
        def __init__(self):
            self.closed = False
...
def test_celery_deliver_notification_missing(monkeypatch):
    """Test case for test celery deliver notification missing."""
    class FakeSession:
        def __init__(self):
            self.closed = False
...
def test_celery_deliver_notification_calls_email_and_push(
    ...
    """Test case for test celery deliver notification calls email and push."""
    # Arrange: create a notification with prefs enabling email/push.
    n = notification_models.Notification(
        user_id=test_user["id"],
        content="hi",
        notification_type="t",
    ...
    # Act/Assert: both email and push paths are triggered once.
...
def test_celery_deliver_notification_handles_exceptions(
    ...
    """Test case for test celery deliver notification handles exceptions."""
    # Arrange: create notification and force both senders to fail.
    n = notification_models.Notification(
        user_id=test_user["id"],
        content="hi",
        notification_type="t",
    ...
    monkeypatch.setattr(celery_worker, "SessionLocal", lambda: session)
    # Act/Assert: errors are logged, not raised.
    deliver_notification(n.id)
...
def test_calc_user_metrics_creates_and_updates(session, test_user):
    # create analytics
    """Test case for test calc user metrics creates and updates."""
...
def test_calc_user_metrics_peak_hours(session, test_user):
    """Test case for test calc user metrics peak hours."""
