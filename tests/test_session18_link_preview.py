...
def test_extract_link_preview_fallback_title(monkeypatch):
    ...
    monkeypatch.setattr(link_preview.requests, "get", lambda url, timeout: DummyResp(html))
    monkeypatch.setattr(link_preview, "validators", SimpleNamespace(url=lambda u: True))
...
def test_extract_link_preview_non_200_returns_none(monkeypatch):
    ...
    monkeypatch.setattr(link_preview.requests, "get", lambda url, timeout: DummyResp(html, status=500))
    monkeypatch.setattr(link_preview, "validators", SimpleNamespace(url=lambda u: True))
...
def test_blocked_domain_returns_none(monkeypatch):
    monkeypatch.setattr(link_preview, "validators", SimpleNamespace(url=lambda u: True))
    monkeypatch.setattr(link_preview.requests, "get", lambda url, timeout: DummyResp("<html></html>"))
