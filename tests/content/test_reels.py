...
def test_create_reel_endpoint(authorized_client, community):
    """Test case for test create reel endpoint."""
    payload = {
...
def test_list_active_reels_filters_expired(
    ...
    """Test case for test list active reels filters expired."""
...
def test_delete_reel_marks_inactive(authorized_client, session, community, test_user):
    """Test case for test delete reel marks inactive."""
    now = datetime.now(timezone.utc)
    reel = _create_reel(
        session, test_user["id"], community.id, expires_at=now + timedelta(hours=1)
    )
...
def test_increment_view_count(authorized_client, session, community, test_user):
    """Test case for test increment view count."""
    now = datetime.now(timezone.utc)
    reel = _create_reel(
        session, test_user["id"], community.id, expires_at=now + timedelta(hours=1)
    )
