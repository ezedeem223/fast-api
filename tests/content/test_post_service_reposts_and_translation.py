"""Test module for test session20 post service."""
...
def test_create_post_increments_count_and_invalidates_cache(session, monkeypatch):
    """Test case for test create post increments count and invalidates cache."""
    user = models.User(
...
def test_update_post_forbidden_for_non_owner(session, monkeypatch):
    """Test case for test update post forbidden for non owner."""
    owner = models.User(
    ...
    assert exc.value.detail == "Not authorized to perform requested action"


def test_poll_vote_duplicate_and_closed(session, monkeypatch):
    """Test case for test poll vote duplicate and closed."""
    user = models.User(
...
def test_repost_updates_stats_and_notifications(session, monkeypatch):
    """Test case for test repost updates stats and notifications."""
    # Arrange: create original post and reposter with stubbed side effects.
    owner = models.User(
    ...
    # Assert: repost updates counts and triggers notifications.
    session.refresh(post)
...
def test_repost_community_scope_notifies_members(session, monkeypatch):
    """Test case for test repost community scope notifies members."""
    # Arrange: create a community-scoped post and member.
    owner = models.User(
    ...
    # Assert: community notification path is used.
...
def test_get_post_applies_translation(session, monkeypatch):
    """Test case for test get post applies translation."""
    user = models.User(
