"""Test module for test session2 post service extra."""
import pytest
...
def test_create_post_rejects_banned_words(session, monkeypatch):
    """Test case for test create post rejects banned words."""
...
def test_create_post_flags_offensive_content(session, monkeypatch):
    """Test case for test create post flags offensive content."""
    service = PostService(session)
...
def test_prepare_post_response_sets_privacy_default(session):
    """Test case for test prepare post response sets privacy default."""
    service = PostService(session)
