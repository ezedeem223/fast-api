"""Test module for test session16 content routers."""
...
def test_post_create_requires_verified_and_nonempty(session):
    """Test case for test post create requires verified and nonempty."""
    ...
    if resp.status_code == 403:
        assert resp.json()["detail"] == "User is not verified."
    else:
        assert resp.json()["detail"] == "Content cannot be empty"
    ...
    assert resp2.json()["title"] == "t"


def test_comment_create_and_permissions(session):
    """Test case for test comment create and permissions."""
    ...
    if resp_forbidden.status_code == 403:
        assert resp_forbidden.json()["detail"] == "Not authorized to delete this comment"
    else:
        assert resp_forbidden.json()["detail"] == "Comment not found"


def test_community_create_requires_verified_and_join_private(session):
    """Test case for test community create requires verified and join private."""
    unverified = _user(session, verified=False)
    client = make_client(session, current_user=unverified)
    resp = client.post("/communities/", json={"name": "C", "description": "d"})
    assert resp.status_code == 403
    assert resp.json()["detail"] == "Your account must be verified to create a community"
    ...
    if join_resp.status_code == 200:
        assert join_resp.json()["message"] == "Successfully joined the community"
    else:
        assert (
            join_resp.json()["detail"]
            == "This is a private community and requires an invitation to join"
        )


def test_reaction_invalid_input(session):
    """Test case for test reaction invalid input."""
    ...
    if resp.status_code == 422:
        ...
        assert "reaction" in str(detail).lower()


def test_sticker_upload_invalid_extension(session, tmp_path, monkeypatch):
    """Test case for test sticker upload invalid extension."""
    ...
    if resp.status_code == 400:
        assert resp.json()["detail"] == "Only PNG and WEBP formats are allowed"
