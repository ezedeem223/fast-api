"""Test module for test post translation flag."""
import os
...
def sample_post(session, test_user):
    """Pytest fixture for sample_post."""
    post = models.Post(
        title="Hola",
        content="Hola mundo",
...
def enable_translation_flag():
    """Pytest fixture for enable_translation_flag."""
...
async def test_list_posts_translates_when_flag_enabled(session, test_user, sample_post):
    """Test case for test list posts translates when flag enabled."""
...
async def test_list_posts_skips_translation_when_flag_disabled(
    ...
    """Test case for test list posts skips translation when flag disabled."""
    service = PostService(session)
    user = session.get(models.User, test_user["id"])
    calls = []
