...
def make_client(db_session):
    ...
    app.dependency_overrides[community_router.oauth2.get_current_user] = lambda: db_session.query(models.User).first()
...
def test_join_requires_existing_invite_for_private():
    with TestingSessionLocal() as db:
        ...
        community = models.Community(name="Private", description="d", owner_id=owner.id, is_private=True)
        community.members.append(models.CommunityMember(user_id=owner.id, role=models.CommunityRole.OWNER))
        db.add(community)
        db.commit()
        db.refresh(community)

        client = make_client(db)
        client.app.dependency_overrides[community_router.oauth2.get_current_user] = lambda: joiner

        resp = client.post(f"/communities/{community.id}/join")
        assert resp.status_code == status.HTTP_403_FORBIDDEN

        invite = models.CommunityInvitation(
            community_id=community.id, inviter_id=owner.id, invitee_id=joiner.id, status="pending"
        )
        db.add(invite)
        db.commit()
...
def test_invite_and_accept_flow():
    with TestingSessionLocal() as db:
        ...
        client = make_client(db)
        client.app.dependency_overrides[community_router.oauth2.get_current_user] = lambda: owner

        resp = client.post(f"/communities/{community.id}/invite", json={"user_id": invitee.id})
        assert resp.status_code == 200
        invitation_id = resp.json()["id"]

        client.app.dependency_overrides[community_router.oauth2.get_current_user] = lambda: invitee
        accept = client.post(f"/communities/invitations/{invitation_id}/accept")
        assert accept.status_code == 200
        member = db.query(models.CommunityMember).filter_by(user_id=invitee.id, community_id=community.id).first()
...
def test_get_communities_sorted():
    with TestingSessionLocal() as db:
        ...
        client = make_client(db)
        for idx in range(3):
            comm = models.Community(name=f"Comm{idx}", description="d", owner_id=user.id)
            db.add(comm)
        db.commit()
