"""Test module for test session22 community service."""
import pytest
...
def _service(session, monkeypatch):
    """Helper for  service."""
    monkeypatch.setattr(
        community_service, "check_content_against_rules", lambda *a, **k: True
    )
    monkeypatch.setattr(community_service, "check_for_profanity", lambda *a, **k: False)
    monkeypatch.setattr(
        community_service,
        "get_translated_content",
        lambda content, user, lang=None: content,
    )
...
def test_owner_can_promote_moderator(session, monkeypatch):
    """Test case for test owner can promote moderator."""
    owner = models.User(
        email="owner22@example.com", hashed_password="x", is_verified=True
    )
    member = models.User(
        email="member22@example.com", hashed_password="x", is_verified=True
    )
    ...
    member_row = models.CommunityMember(
        user_id=member.id, community_id=community.id, role=models.CommunityRole.MEMBER
    )
    owner_row = models.CommunityMember(
        user_id=owner.id, community_id=community.id, role=models.CommunityRole.OWNER
    )
...
def test_invite_accept_decline(session, monkeypatch):
    """Test case for test invite accept decline."""
    # Arrange: create community owner and invitee.
    owner = models.User(
        email="owner223@example.com", hashed_password="x", is_verified=True
    )
    invitee = models.User(
        email="invitee22@example.com", hashed_password="x", is_verified=True
    )
    community = models.Community(name="inv22", description="d", owner=owner)
    session.add_all([owner, invitee, community])
    session.commit()
    owner_member = models.CommunityMember(
        user_id=owner.id, community_id=community.id, role=models.CommunityRole.OWNER
    )
    ...
    # Act: invite a new user and decline.
    non_member = models.User(
        email="new22@example.com", hashed_password="x", is_verified=True
    )
...
def test_archive_and_unarchive(session, monkeypatch):
    """Test case for test archive and unarchive."""
    owner = models.User(
        email="owner224@example.com", hashed_password="x", is_verified=True
    )
    ...
    member = models.User(
        email="mem22@example.com", hashed_password="x", is_verified=True
    )
    session.add(member)
    session.commit()
    session.add(
...
def test_update_member_role_rejects_owner_role(session, monkeypatch):
    """Test case for test update member role rejects owner role."""
    owner = models.User(
        email="owner22x@example.com", hashed_password="x", is_verified=True
    )
    member = models.User(
        email="member22x@example.com", hashed_password="x", is_verified=True
    )
...
def test_update_community_statistics_counts(session, monkeypatch):
    """Test case for test update community statistics counts."""
    # Arrange: create community with members, post, comment, and vote.
    owner = models.User(
        email="owner22stats@example.com", hashed_password="x", is_verified=True
    )
    member = models.User(
        email="member22stats@example.com", hashed_password="x", is_verified=True
    )
    ...
    comment = models.Comment(
        owner_id=member.id, post_id=post.id, content="hi", language="en"
    )
    session.add(comment)
    session.add(models.Vote(user_id=member.id, post_id=post.id))
    session.commit()

    # Act/Assert: statistics reflect the seeded data.
    svc = _service(session, monkeypatch)
