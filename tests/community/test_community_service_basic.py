"""Test module for test community service basic."""
import pytest
...
def test_create_community_success(session):
    """Test case for test create community success."""
...
def test_create_community_unverified_rejected(session):
    """Test case for test create community unverified rejected."""
    ...
    assert (
        exc.value.detail == "Your account must be verified to create a community"
    )


def test_create_community_respects_owned_limit(session, monkeypatch):
    """Test case for test create community respects owned limit."""
    user = _make_user(session)
    service = CommunityService(session)
    ...
    assert exc.value.detail == "You cannot create more than 1 communities"


def test_get_communities_filters_and_sorts(session):
    """Test case for test get communities filters and sorts."""
    user = _make_user(session)
    service = CommunityService(session)
