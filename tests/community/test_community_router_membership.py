"""Test module for test session6 community router."""
...
def test_create_community_with_rules_persists(authorized_client):
    """Test case for test create community with rules persists."""
    payload = {
        "name": "Rule Hub",
...
def test_duplicate_name_rejected(authorized_client):
    """Test case for test duplicate name rejected."""
...
def test_owner_limit_enforced(monkeypatch, authorized_client):
    """Test case for test owner limit enforced."""
    monkeypatch.setattr(
    ...
    assert second.json()["detail"] == "You cannot create more than 1 communities"


def test_invitation_send_and_accept_adds_member(authorized_client, client, test_user2):
    """Test case for test invitation send and accept adds member."""
...
def test_expired_invitation_rejected(authorized_client, client, test_user2):
    """Test case for test expired invitation rejected."""
    community_resp = authorized_client.post(
...
def test_archive_blocks_new_posts(authorized_client):
    """Test case for test archive blocks new posts."""
    community_resp = authorized_client.post(
    ...
    assert (
        post_attempt.json()["detail"]
        == "Community is archived and cannot accept new posts"
    )


def test_unarchive_restores_posting(authorized_client):
    """Test case for test unarchive restores posting."""
    community_resp = authorized_client.post(
    ...
    assert post_resp.json()["content"] == "Welcome back"


def test_duplicate_archive_conflict(authorized_client):
    """Test case for test duplicate archive conflict."""
    community_resp = authorized_client.post(
    ...
    assert second.json()["detail"] == "Community is already inactive"
