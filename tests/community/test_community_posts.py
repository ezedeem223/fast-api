"""Test module for test session2 community posts."""
import asyncio
import warnings
...
def test_create_community_applies_tags_and_rules(session):
    """Test case for test create community applies tags and rules."""
    service = CommunityService(session)
    owner = _make_user(session, "owner-tags@example.com")
    tag = models.Tag(name="tag-one")
    session.add(tag)
    session.commit()
    session.refresh(tag)
    ...
    payload = schemas.CommunityCreate(
        ...
        description="D1",
        tags=[tag.id],
        rules=[schemas.CommunityRuleCreate(rule="be kind")],
    )
    community = service.create_community(current_user=owner, payload=payload)
    assert any(t.id == tag.id for t in community.tags)
    assert any(rule.rule == "be kind" for rule in community.rules)


def test_community_post_empty_content(session):
    """Test case for test community post empty content."""
    service = CommunityService(session)
    ...
    assert exc.value.detail == "Post content cannot be empty"


def test_join_private_community_without_invite(session):
    """Test case for test join private community without invite."""
    service = CommunityService(session)
    ...
    assert (
        exc.value.detail
        == "This is a private community and requires an invitation to join"
    )


def test_service_invite_and_accept_flow(session):
    """Test case for test service invite and accept flow."""
    service = CommunityService(session)
...
def test_invite_quota_exceeded(session, monkeypatch):
    """Test case for test invite quota exceeded."""
    service = CommunityService(session)
    ...
    assert exc.value.status_code == 400
    assert exc.value.detail == "Cannot send more than 1 pending invitations"


def test_create_community_duplicate_name_case_insensitive(session):
    """Test case for test create community duplicate name case insensitive."""
    service = CommunityService(session)
    owner = _make_user(session, "owner-limit@example.com")
    service.create_community(
        current_user=owner,
        payload=schemas.CommunityCreate(name="Case", description="d"),
    )
    with pytest.raises(HTTPException) as exc:
        service.create_community(
            current_user=owner,
...
def test_community_post_rule_violation(session, monkeypatch):
    """Test case for test community post rule violation."""
    service = CommunityService(session)
...
def test_community_vip_upgrade(session, monkeypatch):
    """Test case for test community vip upgrade."""
    service = CommunityService(session)
...
def test_post_service_validation_and_analysis(session):
    """Test case for test post service validation and analysis."""
...
def test_comment_delete_requires_owner(session):
    """Test case for test comment delete requires owner."""
...
def test_business_registration_and_transaction(session):
    """Test case for test business registration and transaction."""
...
def test_user_service_language_and_duplicates(session):
    """Test case for test user service language and duplicates."""
    user_service = UserService(session)
    new_user = user_service.create_user(
        schemas.UserCreate(
            email="dup@example.com", password="StrongPass123!", username="u1"
        )
    )
    with pytest.raises(HTTPException):
        user_service.create_user(
            schemas.UserCreate(
                email="dup@example.com", password="StrongPass123!", username="u2"
            )
        )
...
def test_search_statistics_and_suggestions(session):
    """Test case for test search statistics and suggestions."""
...
def test_encrypted_session_routes(session):
    ...
    """Test case for test encrypted session routes."""
    class DummySignal:
        def __init__(self):
            self.root_key = b"r"
            self.chain_key = b"c"
    ...
    u1 = user_service.create_user(
        schemas.UserCreate(
            email="sess1@example.com", password="StrongPass123!", username="u1"
        )
    )
    u2 = user_service.create_user(
        schemas.UserCreate(
            email="sess2@example.com", password="StrongPass123!", username="u2"
        )
    )
